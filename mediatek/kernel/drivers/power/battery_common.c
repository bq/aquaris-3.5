...
int charger_hv_detect_sw_thread_handler(void *unused)
{
	...
	kal_uint32 hv_voltage = BATTERY_VOLT_10_500000_V;
	kal_bool hv_status;	
...
#if 0
	battery_xlog_printk(BAT_LOG_CRTI, "[%s] hv set start\n", __FUNCTION__);
	if(chargin_hw_init_done)
	{
		kal_uint32 hv_voltage = BATTERY_VOLT_10_000000_V;
		battery_charging_control(CHARGING_CMD_SET_HV_THRESHOLD, &hv_voltage);
	}
	battery_xlog_printk(BAT_LOG_CRTI, "[%s] hv set end\n", __FUNCTION__);
#endif
...
static int battery_probe(struct platform_device *dev)    
{
	...
	if(1)
	{
		...
		battery_charging_control(CHARGING_CMD_GET_HV_STATUS,&hv_status);
		battery_xlog_printk(BAT_LOG_CRTI, "battery_probe1 : hv_status %d\n",hv_status);
		battery_charging_control(CHARGING_CMD_SET_HV_THRESHOLD,&hv_voltage);
		battery_charging_control(CHARGING_CMD_GET_HV_STATUS,&hv_status);
		battery_xlog_printk(BAT_LOG_CRTI, "battery_probe2 : hv_status %d\n",hv_status);
	}
