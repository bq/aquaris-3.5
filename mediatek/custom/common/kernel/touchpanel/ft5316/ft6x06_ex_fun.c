...
extern struct i2c_client *i2c_client;//xuqian add

/* -- dirver configure -- */
/*register address*/
#define FT6x06_REG_FW_VER		0xA6

/*xuqian add for tp firmware auto upgrade with bin file*/
#define FT6x06_REG_VENDOR_VER		0xA8//xuqian add

#define FTS_CTP_VENDOR_LIANGCHUANG   0x87
#define FTS_CTP_VENDOR_JINGLONG          0x98
//#define FTS_CTP_VENDOR_YASHI                0x11
#define FTS_CTP_VENDOR_WALLY     0x11

#define FW_OF_YASHI                "FT6206_EASTAEON_yashi_app.bin"
#define FW_OF_JINGLONG          "FT6206_EASTAEON_jinglong_app.bin"
#define FW_OF_LIANGCHUANG   "FT6206_EASTAEON_liangchuang_app.bin"
#define FW_OF_WALLY                "FT6206_EASTAEON_wally_app.bin"

static char *fts_get_vendor_name(u8 vendor_id)
{
	switch(vendor_id){
		//case FTS_CTP_VENDOR_YASHI:                       return "YASHI";break;
		case FTS_CTP_VENDOR_JINGLONG:		       return "JINGLONG"; break;
		case FTS_CTP_VENDOR_LIANGCHUANG:		return "LIANGCHUANG";break;
		case FTS_CTP_VENDOR_WALLY:                      return "WALLY";break;
		default:							return "UNKNOWN";	break;
	}
	return "UNKNOWN";
}

static char *fts_get_firmware_name(u8 vendor_id)
{
	switch(vendor_id){
		//case FTS_CTP_VENDOR_YASHI:                       return FW_OF_YASHI;break;
		case FTS_CTP_VENDOR_JINGLONG:		       return FW_OF_JINGLONG;break;
		case FTS_CTP_VENDOR_LIANGCHUANG:		return FW_OF_LIANGCHUANG;break;
		case FTS_CTP_VENDOR_WALLY:		              return FW_OF_WALLY;break;
...
static unsigned char CTPM_FW[] = {
	//#include "ft_app.i"
	//#include "malata_n350_ID59_Ver12_20130707_app.i"
	#include "malata_n350_ID59_Ver18_20130815_app.i"
};

static unsigned char CTPM_FW_Lianchuang[] = {
	//#include "FT6206_Eastaeon_V795_Lianchuang_LCFV035817_0x14_20130606_app.i"
};
/*
static unsigned char CTPM_FW_Yashi[] = {
	#include "FT6206_EASTAEON_V795_KOTL_0350_0x13_20130523_app.i"
};
*/
static unsigned char CTPM_FW_JingLong[] = {
	//#include "FT6206_EASTAEON_V795_KOTL_0350_0x13_20130523_app.i"
};
static unsigned char CTPM_FW_Wally[] = {
	#include "HRC_M526_0x03_20130726_app.i"
};
...
/*xuqian add for upgrade with i file use sys attribute*/
int fts_ctpm_fw_upgrade_with_i_file_4_sys(struct i2c_client *client, char *CTPM_FW, int len)
{
	u8 *pbt_buf = NULL;
	int i_ret;
	int fw_len;
	char *CTPM_FW_tail = CTPM_FW + len;
	...
	*/
	...
	if ((*(CTPM_FW_tail - 8) ^ *(CTPM_FW_tail - 6)) == 0xFF
		&& (*(CTPM_FW_tail - 7) ^ *(CTPM_FW_tail - 5)) == 0xFF
		&& (*(CTPM_FW_tail - 3) ^ *(CTPM_FW_tail - 4)) == 0xFF){
		/*FW upgrade */
		pbt_buf = CTPM_FW;
		/*call the upgrade function */
...
u8 fts_ctpm_get_i_file_ver_4_sys(char *CTPM_FW, int len)
{
	u16 ui_sz;
	ui_sz = len;
	u8 vendor_id;
	...
	vendor_id = *(CTPM_FW + len -1);

	vendor_name = fts_get_vendor_name(vendor_id);

	fw_ver = *(CTPM_FW + len -2);

	if (ui_sz > 2)
	{
		DBG("xuqian the i file is: %s, version: %d,%s, %d\n", vendor_name, fw_ver, __FUNCTION__, __LINE__);
		return fw_ver;
	}
...
/*xuqian add auto update with i file*/
int fts_ctpm_auto_upgrade_with_i_file(struct i2c_client *client)
{
	u8 uc_host_fm_ver = FT6x06_REG_FW_VER;
	int i_ret;

	int fw_len;
	char *CTPM_FW_bin = NULL;
	//int yashi_fw_len = sizeof(CTPM_FW_Yashi);
	int jinglong_fw_len = sizeof(CTPM_FW_JingLong);
	int liangchuang_fw_len = sizeof(CTPM_FW_Lianchuang);
	int wally_fw_len = sizeof(CTPM_FW_Wally);
	u8 tp_vendor_ver = 0;
	u8 tp_fw_ver;
	u8 i_file_fw_ver;
	...
	ft5x0x_read_reg(client, FT6x06_REG_FW_VER, &tp_fw_ver);
	ft5x0x_read_reg(client, FT6x06_REG_VENDOR_VER, &tp_vendor_ver);
	
	switch(tp_vendor_ver){
		/*
		case FTS_CTP_VENDOR_YASHI:
			DBG("xuqian TP vendor is: yashi %s, %d\n", __FUNCTION__, __LINE__);
			fw_len = yashi_fw_len;
			CTPM_FW_bin = CTPM_FW_Yashi;
			break;
			*/
		case FTS_CTP_VENDOR_JINGLONG:
			DBG("xuqian TP vendor is: jinglong %s, %d\n", __FUNCTION__, __LINE__);
			fw_len = jinglong_fw_len;
			CTPM_FW_bin = CTPM_FW_JingLong;
			break;
		case FTS_CTP_VENDOR_LIANGCHUANG:
			DBG("xuqian TP vendor is: liangchuang %s, %d\n", __FUNCTION__, __LINE__);
			fw_len = liangchuang_fw_len;
			CTPM_FW_bin = CTPM_FW_Lianchuang;
			break;
		case FTS_CTP_VENDOR_WALLY:
			DBG("xuqian TP vendor is: huaruichuan %s, %d\n", __FUNCTION__, __LINE__);
			fw_len = wally_fw_len;
			CTPM_FW_bin = CTPM_FW_Wally;
			break;
		default:
			DBG("xuqian TP vendor is: UNKNOWN %s, %d\n", __FUNCTION__, __LINE__);
			return "UNKNOWN";	break;
	}

	if(fw_len == 0)
	{
		DBG("Firmware is null, do nothing %s, %d<<<<<<<<\n", __FUNCTION__, __LINE__);
		return -1;
	}
	i_file_fw_ver = fts_ctpm_get_i_file_ver_4_sys(CTPM_FW_bin, fw_len);
	...
	if (i_file_fw_ver > tp_fw_ver) 
		...
		i_ret = fts_ctpm_fw_upgrade_with_i_file_4_sys(client, CTPM_FW_bin, fw_len);
		if (i_ret == 0)	{
			msleep(300);
...
int fts_ctpm_auto_upgrade_app(struct i2c_client *client)
{
	...
	//FT6x06_REG_FW_VER
	//FT6x06_REG_VENDOR_VER
    /*get tp info vendor & firmware version*/
	ft5x0x_read_reg(client, FT6x06_REG_FW_VER, &tp_fm_ver);
	ft5x0x_read_reg(client, FT6x06_REG_VENDOR_VER, &tp_vendor_ver);
	vendor_name = fts_get_vendor_name(tp_vendor_ver);
	if(vendor_name != NULL)
	{
		DBG("xuqian tp_vendor_ver = %d,\n vendor_name = %s,\n line = %d\n",\
		tp_vendor_ver, vendor_name, __LINE__);
	}

	/*get firmware bin file name according to tp vendor*/
	firmware_name = fts_get_firmware_name(tp_vendor_ver);
	if(!firmware_name)
	{
		DBG("xuqian firmware of vendor_name = %s not found line = %d\n",\
		vendor_name, __LINE__);
		return -1;
	}
	...
	/*read firmware bin file from sdcard*/
	...
	/*get firmware bin version*/
	 bin_fm_ver = *(pbt_buf + fwsize - 2);
	
	/*compare tp & bin file firmware version*/
	 if(bin_fm_ver <= tp_fm_ver)
	{
		DBG("xuqian %s,  it's already the latest firmware no need to upgrade, exit!!!%d\n", __FUNCTION__, __LINE__);
		return 0;
	 }
...
/* xuqian add to show tp vendor */
static ssize_t ft6x06_tpvendor_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	ssize_t num_read_chars = 0;
	u8 tpvendor_id = 0;
	char *tpvendor_name = NULL;
	//struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	struct i2c_client *client = i2c_client;
	...
	DBG("xuqian %s, %d\n", __FUNCTION__, __LINE__); 
	DBG("xuqian %s, i2c_client->addr = %x i2c_client->timing =%d line= %d\n", \
		__FUNCTION__, i2c_client->addr, i2c_client->timing, __LINE__);

	if (ft5x0x_read_reg(client, FT6x06_REG_VENDOR_VER, &tpvendor_id) < 0)
	{
		num_read_chars = snprintf(buf, PAGE_SIZE,
					"get tp vendor fail!\n");
	}
	...
	{
		tpvendor_name = fts_get_vendor_name(tpvendor_id);
...
static ssize_t ft6x06_tpvendor_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	/*place holder for future use*/
					...
					const char *buf, size_t count)
{
	...
	//struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	struct i2c_client *client = i2c_client;
	//CTPM_FW_Lianchuang  CTPM_FW_Yashi  CTPM_FW_JingLong
	...
	/*choose which i file to use*/
	if(buf != NULL && count != 0)
	{
/*
		if(buf[0]== '0')
		{
			DBG("xuqian you choose yashi %s, %d\n", __FUNCTION__, __LINE__);
			fw_len = yashi_fw_len;
			CTPM_FW_bin = CTPM_FW_Yashi;
		}
*/
		if(buf[0]== '1')
		{
			DBG("xuqian you choose jinglong %s, %d\n", __FUNCTION__, __LINE__);
			fw_len = jinglong_fw_len;
			CTPM_FW_bin = CTPM_FW_JingLong;
		}
		if(buf[0]== '2')
		{
			DBG("xuqian you choose liangchuang %s, %d\n", __FUNCTION__, __LINE__);
			fw_len = liangchuang_fw_len;
			CTPM_FW_bin = CTPM_FW_Lianchuang;
		}
	}
	...
	if (i_ret == 0) {
		msleep(300);
		uc_host_fm_ver = fts_ctpm_get_i_file_ver_4_sys(CTPM_FW_bin, fw_len);
		pr_info("%s [FTS] upgrade to new version 0x%x\n", __func__,
					 uc_host_fm_ver);
	} else
		dev_err(&client->dev, "%s ERROR:[FTS] upgrade failed.\n",
...
/* xuqian add to show tp vendor */
/* get the tp vendor info
  * example: cat ftstpvendor
  */
static DEVICE_ATTR(ftstpvendor, S_IRUGO | S_IWUSR, ft6x06_tpvendor_show,
			ft6x06_tpvendor_store);
...
static struct attribute *ft5x0x_attributes[] = {
	...
	&dev_attr_ftstpvendor.attr,//xuqian add to show tp vendor
...
/* xuqian add for sysfs */
int my_ft5x0x_create_sysfs(struct kobject *kobj)
{
	int err;
	err = sysfs_create_group(kobj, &ft5x0x_attribute_group);
	if (0 != err) {
		dev_err(kobj,
					 "%s() - ERROR: sysfs_create_group() failed.\n",
					 __func__);
		sysfs_remove_group(kobj, &ft5x0x_attribute_group);
		return -EIO;
	} else {
		mutex_init(&g_device_mutex);
		pr_info("ft5x0x:%s() - sysfs_create_group() succeeded.\n",
