...
#define	KXTIK_KXCJK_AUTO_DETECT		1
/*----------------------------------------------------------------------------*/

/*----------------------------------------------------------------------------*/
static const struct i2c_device_id kxtik1004_i2c_id[] = {{KXTIK1004_DEV_NAME,0},{}};
static struct i2c_board_info __initdata i2c_kxtik1004={ I2C_BOARD_INFO(KXTIK1004_DEV_NAME, (0x2E>>1))};
...
#if (KXTIK_KXCJK_AUTO_DETECT)
typedef enum{
	KXTIK1004 = 1,
	KXCJK1013,
}KX_SENSOR_TYPE;
static int g_kx_sensor_type = 0;
#endif
/*----------------------------------------------------------------------------*/
typedef enum {
    ADX_TRC_FILTER  = 0x01,
...
/*----------------------------------------------------------------------------*/
#if (KXTIK_KXCJK_AUTO_DETECT)
static int KX_GetDeviceID(struct i2c_client *client,u8 *id)
{
//	u8 databuf[10];    
	int res = 0;

//	memset(databuf, 0, sizeof(u8)*10);
	res = hwmsen_read_byte(client, KXTIK1004_REG_DEVID, id);
	GSE_ERR("%s: id = 0x%x\n",__FUNCTION__,*id);
...
static int KXTIK1004_CheckDeviceID(struct i2c_client *client)
{
	...
	if(KXTIK1004_SUCCESS != KX_GetDeviceID(client,&id))
	{
		return KXTIK1004_ERR_I2C;
	}

	if(id == KXTIK1004_DEVICE_ID)
	{
		return KXTIK1004_SUCCESS;
	}
	return KXTIK1004_ERR_I2C;
...
static int KXCJK1013_CheckDeviceID(struct i2c_client *client)
{
	u8 id = 0;
...
static int KXTIK1004_ReadChipInfo(struct i2c_client *client, char *buf, int bufsize)
{
	...
	#if (KXTIK_KXCJK_AUTO_DETECT)
	if(g_kx_sensor_type == KXCJK1013)
	{
		sprintf(buf, "KXCJK1013 Chip");
	}
	else
	{
...
#if (KXTIK_KXCJK_AUTO_DETECT)
static int KXTIK_KXCJK_auto_detect(struct i2c_client *client)
{
	int res = 0;

	client->addr = KXTIK1004_I2C_SLAVE_ADDR>>1;
	res = KXTIK1004_CheckDeviceID(client); 
	if(res == KXTIK1004_SUCCESS)
	{
		g_kx_sensor_type = KXTIK1004;
		printk("%s,g_kx_sensor_type = 0x%x!\n ",__FUNCTION__,g_kx_sensor_type);
		return res;
	}	
	
	client->addr = KXCJK1013_I2C_SLAVE_ADDR>>1;
	res = KXCJK1013_CheckDeviceID(client); 
	if(res == KXTIK1004_SUCCESS)
	{
		g_kx_sensor_type = KXCJK1013;
		printk("%s,g_kx_sensor_type = 0x%x!\n ",__FUNCTION__,g_kx_sensor_type);
		return res;
	}	
...
static int kxtik1004_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//kxtik1004_i2c_client->addr = KXTIK1004_I2C_SLAVE_ADDR;
	#if (KXTIK_KXCJK_AUTO_DETECT)
	if((err = KXTIK_KXCJK_auto_detect(new_client)))
	{
